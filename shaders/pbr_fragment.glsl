...
void main(){
    ...
    // Calcular F0: para dieléctricos (0.04) o basados en el albedo si es metálico.
    vec3 F0 = mix(vec3(0.04), albedoColor, metallic);
    vec3 V = normalize(camPos - FragPos);
    
    vec3 result = vec3(0.0);
    
    // Contribución de las luces puntuales
    for (int i = 0; i < NUM_POINT_LIGHTS; ++i) {
        ...
        result += (kD * albedoColor / PI + specular) * pointLights[i].color * NdotL;
    }
    
    // Contribución de las luces focales (spotlights)
    for (int i = 0; i < NUM_SPOT_LIGHTS; ++i) {
        vec3 L = normalize(spotLights[i].position - FragPos);
        vec3 H = normalize(V + L);
        float NdotL = max(dot(N, L), 0.0);
        
        // Calcular intensidad basada en el ángulo del foco
        ...
        result += intensity * ((kD * albedoColor / PI + specular) * spotLights[i].color * NdotL);
    }
